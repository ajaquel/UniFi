...
# Version               | 5.2.2
# Application version   | 5.7.23-10670
...
if [[ "${script_option_custom_url}" != 'true' ]]; then
  ...
  if wget "${wget_progress[@]}" -O "$unifi_temp" "https://dl.ui.com/unifi/debian/pool/ubiquiti/u/unifi/unifi_${unifi_secret}_all.deb" &>> "${eus_dir}/logs/unifi_download.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded application version ${unifi_clean}! \\n"; elif wget "${wget_progress[@]}" -O "$unifi_temp" "https://dl.ui.com/unifi/debian/pool/ubiquiti/u/unifi/unifi_${unifi_clean}_all.deb" &>> "${eus_dir}/logs/unifi_download.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded application version ${unifi_clean}! \\n"; else echo -e "${RED}#${RESET} Failed to download application version ${unifi_clean}...\\n"; abort; fi
else
